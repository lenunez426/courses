...
\begin{center}
    \begin{tikzpicture}[
        declare function={f(\x)= 60 - 9.8*\x^2/(2*(28.6^2);}
    ]
        ...
        \fill (0,{f(0)}) circle (3pt);
        
        \pgfplotsinvokeforeach{20,40,60,80,100}{
            \fill (#1,{f(#1)}) circle (3pt);
            \draw[dashed] (#1,{f(0)}) -- ++(0,{-(f(0)-f(#1))});
            \fill[cyan] (#1,{f(0)}) circle (3pt);
            \draw[dashed] (0,{f(#1)}) -- ++(#1,0);
            \fill[red] (0,{f(#1)}) circle (3pt);
        }
...
\begin{center}
    ...
    \pgfplotsset{compat=1.11}
        ...
        \draw[dashed,domain=0:16,variable=\x,samples=200] plot ({\x},{f(\x)}) node[above left=-4pt] {\faSoccerBallO};
...
\subsection{Solve Problems Involving Projectile Motion}
...
\Solution Our strategy is that the motion can be broken into horizontal and vertical motions in which $a_x = 0$ and $a_y = \SI{-9.80}{m/s^2}$. Let's define given variables and the unknown. First, define initial positions as $x_0 = 0$ and $y_0 = 0$. Thus, the cannonball's final vertical position will be $y = -\SI{12.0}{m}$ (the negative sign indicating that the projectile will be displacement downwards at the moment of impact).
...
Therefore, the takes the cannonball 1.56 seconds after being launched by the cannon to strike the ground below. 
...
\begin{center}
    \begin{tikzpicture}[
        declare function={f(\x)= 12 - 9.8*\x^2/(2*(18^2);}
    ]
        ...
        \draw[dashed,domain=0:28.15,variable=\x,samples=200] plot ({\x},{f(\x)});
        \fill (0,{f(0)}) circle (2pt) node[left] {$(0,0)$};
        \fill (28.15,{f(28.15)}) circle (2pt) node[below] {$(28.2,-12)$};
        \draw[thick,->] (0,{f(0)}) -- (28,{f(28)}) node[pos=0.5,below left] {$d$};
        \end{axis}
    \end{tikzpicture}
\end{center}
...
\begin{center}
    \begin{tikzpicture}[
        ...
        \draw[dashed,domain=0:28.15,variable=\x,samples=200] plot ({\x},{f(\x)});
        \fill (28.15,{f(28.15)}) circle (2pt);
        \draw[thick,->,red] (28.15,{f(28.15)}) -- ++(axis direction cs: 0.3*18,-0.3*15.3) node[right,black] {$v = \SI{23.6}{m/s}$};
        \end{axis}
    \end{tikzpicture}
\end{center}
